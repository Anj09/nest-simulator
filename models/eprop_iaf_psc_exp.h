...
Current-based leaky integrate-and-fire neuron model with exponential-shaped
postsynaptic currents for e-prop plasticity

Description
+++++++++++

``eprop_iaf_psc_exp`` is an implementation of a leaky integrate-and-fire
neuron model with exponential-shaped postsynaptic currents used for
eligibility propagation (e-prop) plasticity.

The neuron dynamics (excluding e-prop plasticity) are similar to the
``iaf_psc_exp`` model. The e-prop machinery (surrogate gradient, learning
signal, eligibility trace, firing rate regularization) follows the
``eprop_iaf_psc_delta`` implementation.

For full details on e-prop plasticity, see the documentation of
``eprop_iaf_psc_delta``.
...
iaf_psc_exp, eprop_iaf_psc_delta, eprop_synapse
...
/**
 * @brief Class implementing a LIF neuron model with exponential PSCs for e-prop plasticity.
 *
 * Class implementing a current-based leaky integrate-and-fire neuron model with exponential-shaped postsynaptic
 * currents for e-prop plasticity according to Bellec et al. (2020).
 */
...
private:
  ...
  struct Parameters_
  {
    ...
    //! Time constant of excitatory synaptic current (ms).
    double tau_ex_;

    //! Time constant of inhibitory synaptic current (ms).
    double tau_in_;
  ...
  struct State_
  {
    //! Stepwise constant input current (pA).
    double i_0_;

    //! Current input filtered through the excitatory synapse exponential kernel.
    double i_1_;

    //! Postsynaptic current for excitatory inputs (pA).
    double i_syn_ex_;

    //! Postsynaptic current for inhibitory inputs (pA).
    double i_syn_in_;
  ...
  struct Buffers_
  {
    ...
    //! Multi-channel buffer for incoming spikes and currents.
  ...
  struct Variables_
  {
    //! Propagator coefficient for evolving the membrane voltage.
    double P22_;

    //! Propagator coefficient for the contribution of the constant input current to the membrane voltage.
    double P20_;

    //! Propagator coefficient for evolving the excitatory synaptic current.
    double P11ex_;

    //! Propagator coefficient for evolving the inhibitory synaptic current.
    double P11in_;

    //! Propagator coefficient for the contribution of the excitatory synaptic current to the membrane voltage.
    double P21ex_;

    //! Propagator coefficient for the contribution of the inhibitory synaptic current to the membrane voltage.
    double P21in_;

    //! Excitatory weighted spikes accumulated during a time step.
    double weighted_spikes_ex_;

    //! Inhibitory weighted spikes accumulated during a time step.
  ...
  //! Get the current value of the excitatory postsynaptic current.
  double
  get_I_syn_ex_() const
  {
    return S_.i_syn_ex_;
  ...
  //! Get the current value of the inhibitory postsynaptic current.
  double
  get_I_syn_in_() const
  {
    return S_.i_syn_in_;
